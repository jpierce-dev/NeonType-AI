...
export const Header: React.FC<HeaderProps> = memo(({
    ...
    <header className="w-full max-w-7xl px-4 md:px-6 py-3 md:py-4 flex flex-col md:flex-row gap-3 md:gap-4 justify-between items-center z-20">
        <div className="flex items-center gap-3">
            <div className="p-2 bg-gradient-to-br from-neon-blue to-neon-purple rounded-lg shadow-[0_0_15px_rgba(188,19,254,0.4)]">
                <Keyboard className="text-white w-5 h-5 md:w-6 md:h-6" />
            </div>
        ...
        <div className="flex flex-wrap md:flex-row items-center justify-center gap-3 md:gap-4">
            <div className="flex items-center gap-2.5 md:gap-3">
                {/* Mode Switcher */}
                <div className="bg-dark-surface/80 p-1 rounded-lg border border-white/10 flex">
                    <button
                        onClick={() => setMode(GameMode.PRACTICE)}
                        className={`flex items-center gap-1.5 px-3.5 py-1.5 md:px-4 md:py-2 rounded-md text-xs md:text-sm font-bold transition-all ${mode === GameMode.PRACTICE ? 'bg-neon-blue text-black shadow-lg' : 'text-slate-400 hover:text-white'}`}
                    >
                        <Type className="w-4 h-4" /> Practice
                    </button>
                    <button
                        onClick={() => setMode(GameMode.DRILL)}
                        className={`flex items-center gap-1.5 px-3.5 py-1.5 md:px-4 md:py-2 rounded-md text-xs md:text-sm font-bold transition-all ${mode === GameMode.DRILL ? 'bg-neon-purple text-white shadow-lg' : 'text-slate-400 hover:text-white'}`}
                    >
